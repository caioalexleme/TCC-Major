...
include "conexao.php";
    ...
    try
    {
        $Comando=$conexao->prepare("SELECT * FROM manutencao_pessoa");
        ...
        if($Comando->execute())
        {
            ...
            {
                echo '<div id=bloco>';
                echo 'Empresa: '.$linha->nome_pessoa.'<br>';
                echo 'CPF: '.$linha->cpf.'<br>';
                echo 'E-mail: '.$linha->email.'<br>';
                echo 'Telefone: '.$linha->telefone.'<br>';
                echo 'Não conformidade do cliente: '.'<u>'.$linha->descricao.'</u><br>';
                echo '</div>';
            }
        }
        else
